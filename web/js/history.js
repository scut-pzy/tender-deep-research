...
import { createAiMessage } from './chat.js';
    ...
    for (const item of data.history) {
      ...
      el.innerHTML = `
        ...
        <div class="flex items-center justify-between mt-1">
          ...
          <span class="text-[10px] px-1.5 py-0.5 rounded dark:bg-accent/10 bg-accent/10 text-accent">${item.field_count} 个要素</span>
        </div>
      `;
...
async function showHistoryDetail(id) {
  try {
    ...
    // Create AI message with table + JSON
